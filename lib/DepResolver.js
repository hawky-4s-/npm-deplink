...
function compareDependencies(solvedDependencies, cyclicDep, unresolvedDependency) {
  var tempDeps = [];
  ...
  while(!isDependencyResolved && retries > 0) {
    _.some(unresolvedDependencies, function(unresolvedDependency) {
      ...
      if (dependenciesMatch && !unresolvedDependency.hasCyclicDep) {
        ...
        logger.debug('resolveDependency: - > no cyclic dep <-');
        ...
        logger.debug('resolveDependency: - > cyclic dep <-');
        ...
        _.some(unresolvedDependencies, function(cyclicDep) {
          ...
          if (cyclicDep.name != unresolvedDependency.name && cyclicDep.hasCyclicDep) {
            var dependenciesMatch = compareDependencies(solvedDependencies, cyclicDep, unresolvedDependency);
            if (dependenciesMatch) {

              logger.debug('depName: ', cyclicDep.name, '- unresolvedDepName: ', unresolvedDependency.name);
              logger.debug('dependsOn: ', cyclicDep.dependsOn);
              logger.debug('unresolvedDepDependsOn: ', unresolvedDependency.dependsOn);

              var linkCmds = linkFunction(cmdFactory, unresolvedDependency, moduleDependencies, cyclicDep, unresolvedDependency.hasCyclicDep);
              cmds = Array.prototype.concat.apply(cmds, linkCmds);
              ...
              // add them to solved dependencies so we knew they are completed
