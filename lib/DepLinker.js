...
function DependencyLinker(rootDirectory, options) {
  ...
  _.extend(options, { logger: logger });
...
DependencyLinker.prototype.symlinkModule = function(cmdFactory, currentModule, moduleDependencies, cyclicDependency, hasCyclicDep) {
  ...
  if (!cyclicDependency) {
    logger.debug('currentModule: ' + currentModule.name);
  } else {
    logger.debug('currentModule: ' + currentModule.name + ' - cyclicDependency: ' + cyclicDependency.name);
  }
  ...
  if (dependsOn) {
    ...
    if (!dirExistsUnderParentDir(modulePath, NODE_MODULES)) {
      commands.push(cmdFactory.mkDir(path.join(modulePath, NODE_MODULES)));
    }

    _.forEach(currentModule.dependsOn, function (requiredModule) {
      commands.push(cmdFactory.link(
          path.join(moduleDependencies[requiredModule].path),
          path.join(NODE_MODULES, requiredModule),
          FORCE
      ));
    });
  }
  ...
  if (dependsOn && hasCyclicDep) {
    var cyclicDependencyPath = moduleDependencies[cyclicDependency.name].path;
    commands.push(cmdFactory.cd(cyclicDependencyPath));

    if (!dirExistsUnderParentDir(cyclicDependencyPath, NODE_MODULES)) {
      commands.push(cmdFactory.mkDir(path.join(cyclicDependencyPath, NODE_MODULES)));
    }

    _.forEach(cyclicDependency.dependsOn, function(requiredModule) {
      commands.push(cmdFactory.link(
          path.join(moduleDependencies[requiredModule].path),
          path.join(NODE_MODULES, requiredModule),
          FORCE
