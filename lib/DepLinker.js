...
function DependencyLinker(rootDirectory, options) {
  ...
  this._useSymlink = options.symlinks || false;
...
DependencyLinker.prototype.link = function(customCmds, callback) {
  ...
  this.dependencyResolver.discoverDependencies(this._rootDirectory, function(err, result) {
    ...
    var linkFunction = self._useSymlink ? self.symlinkModule : self.npmlinkModule;
...
DependencyLinker.prototype.symlinkModule = function(cmdFactory, currentModule, moduleDependencies, cyclicDependency, hasCyclicDep) {
  ...
  if (!hasCyclicDep) {
    ...
    cmds.push('cd ' + modulePath);
    if (dependsOn) {
      _.remove(currentModule.dependsOn, function(links) {
        return links === cyclicModuleDep.name;
      });
      cmds.push('npm link ' + currentModule.dependsOn.join(' '));
    }
    cmds.push('npm install');

    // second: install and link cyclicDep
    cmds.push('cd ' + moduleDependencies[cyclicModuleDep.name].path);
    if (cyclicModuleDep.dependsOn.length > 0) {
      cmds.push('npm link ' + cyclicModuleDep.dependsOn.join(' '));
    }
    cmds.push('npm install');
